---
title: "hang-in/tunaCtx: Trigger-Driven Dynamic Context Loading for Code-Aware LLM Agents"
source: "https://github.com/hang-in/tunaCtx"
author:
published:
created: 2026-05-05
description: "Trigger-Driven Dynamic Context Loading for Code-Aware LLM Agents - hang-in/tunaCtx"
tags:
  - "clippings"
---
## tunaCtx

원본 [jaytoone/CTX](https://github.com/jaytoone/CTX) 를 production-level 로 리팩토링/보강한 fork. retrieval 알고리즘은 원본 그대로 유지. Claude Code hook 구현이 실제 사용 환경에서 안전하게 운영되도록 모듈 분해, 패키징/설치 정합성, 회귀 가드, 텔레메트리만 손봤음.
...
- **검색 stack**: 단일 알고리즘이 아니라 layer 조합 —
	- **G1 (시간축 기억)**: cross-session decision memory (git HEAD 키 캐시) — 세션을 넘어 과거 결정과 그 이유 회상
		- **G2-DOCS / G2-CODE (공간축 검색)**: BM25 + (옵션) BGE-reranker-v2-m3 **cross-encoder rerank**
		- **chat-memory**: vault.db FTS5 + **vec0 dense (multilingual-e5-small 384-dim) 하이브리드** (α=0.5 cosine + 0.5 bm25)
- **외부 의존성**: LLM API 호출 없음. 로컬 BM25 + (옵션) vec-daemon (multilingual-e5-small) + (옵션) BGE cross-encoder.
- **외부 codebase 측정값 (참고)**: upstream 의 `benchmarks/results/reeval_external_iter11.json` 기준 Mean R@5 = **0.595** (Flask 0.6462 / FastAPI 0.3870 / Requests 0.7526). 본 fork 는 retrieval 알고리즘을 변경하지 않으므로 동일 수치 적용. upstream docs 일부에 잔존하는 `R@5=0.152` 는 pre-fix baseline 으로 stale ([upstream issue #2](https://github.com/jaytoone/CTX/issues/2)).
...
활성화 시 `~/.claude/ctx-telemetry.jsonl` 에 retrieval event 기록 (network upload 없음). 비활성 시 zero-cost early return — orchestrator 모듈 로드 시 gate 1 회 평가 + 호출당 bool 체크 (≈ 0.01µs).
...
- `chat-memory.py:import sqlite_vec` 무방어 → `try/except ImportError` + graceful fallback (sqlite\_vec 부재 환경에서도 hook 죽지 않음)
...
상세 데이터 / 표 / 한계: [`docs/refactor/EVAL_RESULTS.md`](https://github.com/hang-in/tunaCtx/blob/master/docs/refactor/EVAL_RESULTS.md). 한국어 블로그 포스트: [`docs/community/BLOG_POST_eval_ko.md`](https://github.com/hang-in/tunaCtx/blob/master/docs/community/BLOG_POST_eval_ko.md).
...
| `test_chat_memory_fallback.py` | 9 | vault.db 없음, vec-daemon down, sqlite\_vec 부재, invalid stdin |
